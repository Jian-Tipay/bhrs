    ...
    <div class="col-lg-8 mb-4">
      ...
      <div class="card mt-4">
        <div class="card-header">
          ...
          @if($averageRating)
            <div class="mt-2">
              <span class="badge bg-warning">⭐ {{ number_format($averageRating, 1) }}</span>
        ...
        <div class="card-body">
          <div class="row mb-3">
            <div class="col-md-6">
              <p><i class='bx bx-map text-primary'></i> <strong>Address:</strong> {{ $property->address }}</p>
              <p><i class='bx bx-money text-success'></i> <strong>Price:</strong> ₱{{ number_format($property->price, 2) }} / month</p>
              <p><i class='bx bx-door-open text-info'></i> <strong>Rooms:</strong> {{ $property->rooms }}</p>
            </div>
            <div class="col-md-6">
              @if($property->distance_from_campus)
                <p><i class='bx bx-map-pin text-danger'></i> <strong>Distance from Campus:</strong> {{ $property->distance_from_campus }} km</p>
              @endif
              @if($property->capacity)
                <p><i class='bx bx-group text-warning'></i> <strong>Capacity:</strong> {{ $property->capacity }} persons</p>
              @endif
              @if($property->available_slots)
                <p><i class='bx bx-check-circle text-success'></i> <strong>Available Slots:</strong> {{ $property->available_slots }}</p>
              @endif
            </div>
          </div>
          ...
          @if($property->propertyAmenities->count() > 0)
            ...
            <div class="d-flex flex-wrap gap-2">
              @foreach($property->propertyAmenities as $pa)
                <span class="badge bg-label-primary">
                  <i class='bx bx-check'></i> {{ $pa->amenity->amenity_name }}
                </span>
              @endforeach
            </div>
          @endif
      ...
      <div class="card mb-4">
        ...
        <div class="card-body">
          @if($property->owner_name)
            <p><i class='bx bx-user'></i> <strong>Owner:</strong> {{ $property->owner_name }}</p>
          @endif
          @if($property->owner_contact)
            <p><i class='bx bx-phone'></i> <strong>Contact:</strong> {{ $property->owner_contact }}</p>
          @endif
          
          <a href="{{route('bookings.create', ['property' => $property->id]) }}" class="btn btn-primary w-100 mt-2">
              ...
              <div class="alert alert-success mt-3 mb-2 p-3">
                <div class="d-flex justify-content-between align-items-center">
                  <div>
                    ...
                    @if($userRating->review_text)
                      <small class="text-muted d-block mt-1">
                        "{{ Str::limit($userRating->review_text, 50) }}"
                      </small>
                    @endif
                  </div>
                </div>
        ...
        <div class="card-body">
          @foreach($similarProperties as $similar)
            <div class="d-flex mb-3">
              <img src="{{ $similar->image ? asset('storage/' . $similar->image) : asset('assets/img/boarding/default.jpg') }}" 
      ...
      <form action="{{ route('ratings.store') }}" method="POST" id="ratingForm">
        ...
        <div class="modal-body">
          ...
          <div class="d-flex align-items-center mb-4 p-3 bg-light rounded">
            <img src="{{ $property->image ? asset('storage/' . $property->image) : asset('assets/img/boarding/default.jpg') }}" 
            ...
            <div class="d-flex align-items-start">
              ...
              <div>
                ...
                <small>
                  ...
                  @else
                    You have {{ $userRatingsCount }} rating. Just one more to unlock AI-powered recommendations!
                  @endif
                </small>
              </div>
            </div>
...
<div class="modal fade" id="updateRateModal" tabindex="-1" aria-labelledby="updateRateModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <form action="{{ route('ratings.update', $userRating->rating_id) }}" method="POST" id="updateRatingForm">
        @csrf
        @method('PUT')
        ...
        <div class="modal-body">
          ...
          <div class="d-flex align-items-center mb-4 p-3 bg-light rounded">
            ...
            <div class="ms-3">
              <h6 class="mb-0">{{ $property->title }}</h6>
              <small class="text-muted">Rated on {{ $userRating->created_at->format('M d, Y') }}</small>
            </div>
          </div>
          ...
          <div class="mb-4">
            ...
            <div class="star-rating-input-update d-flex justify-content-center gap-2 my-3">
              @for($i = 5; $i >= 1; $i--)
                <input type="radio" name="rating" id="update_star{{ $i }}" value="{{ $i }}" 
                       {{ $userRating->rating == $i ? 'checked' : '' }} required>
                <label for="update_star{{ $i }}" class="star-label" data-rating="{{ $i }}">
                  <i class='bx bxs-star'></i>
                </label>
              @endfor
            </div>
            <div class="text-center">
              <small class="text-muted rating-text-update">
                {{ $userRating->rating }} {{ $userRating->rating == 1 ? 'star' : 'stars' }}
              </small>
            </div>
          </div>
          ...
          <div class="mb-3">
            ...
            <textarea name="review_text" 
                      ...
                      maxlength="1000">{{ $userRating->review_text }}</textarea>
            <div class="form-text">
              <span id="charCountUpdate">{{ strlen($userRating->review_text ?? '') }}</span>/1000 characters
            </div>
          </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  @if(session('success') && str_contains(session('success'), '🎉'))
    setTimeout(() => {
